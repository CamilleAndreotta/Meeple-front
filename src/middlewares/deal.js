...
const dealMiddleware = (store) => (next) => (action) => {
  ...
  switch (action.type) {
    ...
    case FETCH_DEAL: {
      axios.get('http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/deals')
        .then((response) => {
    ...
    case NEW_GAME: {
      ...
      axios.post('http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/new/game',
      {
        name: newGame,
        url: urlGame,
      },
    ...
    case SEARCH_GAME: {
      ...
      axios
        .post(
          "http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/games/byname", {
            name: gameToSearch,
          },)
        .then((response) => {
    ...
    case FETCH_ONE_DEAL: {
      axios
        .get(
          `http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/deals/${action.dealId}`,
    ...
    case FETCH_SHOPS: {
      axios
        .get('http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/shops')
        .then((response) => {
          // Ici on recup bien les données de notre API (les recettes)
          // On veut maintenant les rajouter dans le state
          // Pour ça on va dispatcher une action (l'intention de mémoriser les recettes)
    ...
    case FETCH_NEWS: {
      axios
        .get('http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/articles')
        .then((response) => {
    ...
    case ADD_DEAL_SUMIT_FORM: {
      ...
      axios
        .post(
          'http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/deals', {
            title: dealTitle,
    ...
    case SAVE_COMMENT: {
      ...
      //
      axios.post('http://camilleandreotta-server.eddi.cloud/meeple/current/public/api/comments', {
        comment: userComment,
        deal: id,
        user: parseIntUserId,
      }, {
